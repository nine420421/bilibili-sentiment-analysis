...
def get_chinese_font():
    ...
    try:
        ...
        if chinese_fonts:
            debug_info(f"找到中文字体: {os.path.basename(chinese_fonts[0])}")
...
def create_wordcloud_fixed(word_freq, font_path, width=1000, height=500, max_words=100, colormap='viridis', background_color='white'):
    """修复版词云生成"""
    try:
        # 确保背景颜色正确
        ...
        try:
            ...
            wordcloud = wc_simple.generate_from_frequencies(word_freq)
            debug_info("简化版词云生成成功")
            return wordcloud
        except:
            return None
...
def main():
    ...
    if uploaded_file is not None:
        ...
        with col1:
            max_words = st.slider("最大词汇数量", 50, 200, 100, key="max_words_slider")
        with col2:
            background_color = st.selectbox("背景颜色", 
                ["white", "black", "lightgray", "lightblue"], key="bg_color_selector")
        with col3:
            colormap_option = st.selectbox("颜色方案", 
                ["viridis", "plasma", "spring", "summer", "autumn", "winter", "cool", "hot"], 
        ...
        if st.button("生成词云", type="primary", key="generate_wordcloud"):
            with st.spinner("正在生成词云..."):
                ...
                if all_words:
                    ...
                    # 创建词云
                    wordcloud = create_wordcloud_fixed(
                        word_freq, 
                        font_path, 
                        max_words=max_words,
                    ...
                    if wordcloud is not None:
                        ...
                        ax.set_title(f'{sentiment_option} - 词云图', 
                                   fontsize=22, pad=20, fontweight='bold', color='darkblue')
